...
(function(w,d){
    ...
    RobaxWidget.prototype.postMail=function(){
        //helper.get('//'+hostWidget+'/widget/widget-call',{'key':this.settings.key,'phone':phone,'site_url':window.location.hostname,'protocol':window.location.protocol});
    }
    RobaxWidget.prototype.postCall=function(phone){
        ...
        helper.get('//'+hostWidget+'/widget/widget-call',{'key':this.settings.key,'phone':phone,'event':megaEvent,'site_url':window.location.hostname,'protocol':window.location.protocol},function(r){if(t.dataJSON['widget_yandex_metrika'])window["yaCounter" + t.dataJSON['widget_yandex_metrika']].reachGoal(t.dataJSON['widget_name'],{'phone':phone,'url':window.location.hostname+window.location.pathname});if(window.ga&&t.dataJSON['widget_google_metrika']) window.ga("send", "event", t.dataJSON['widget_name'], 'phone-'+phone+'-url-'+window.location.hostname+window.location.pathname);});
    }
            ...
            while(i<n){
                e[i].onclick=function(){
                    ...
                    switch (this.innerText) {
                        case "Сегодня": {
                            dataTime = getTime(dataJSON["date"]["day"], dataJSON, "Сегодня");
                            var s=Number(dataTime['start'].split(':')[0])*60+Number(dataTime['start'].split(':')[1]),e=Number(dataTime['end'].split(':')[0])*60+Number(dataTime['end'].split(':')[1]),m=e-s,i=0,html='';
                            while (i<=m) {
                        ...
                        case "Завтра": {
                            dataTime = getTime(dataJSON["date"]["day"], dataJSON, "Завтра");
                        ...
                        case "Послезавтра": {
                            dataTime = getTime(dataJSON["date"]["day"], dataJSON, "Послезавтра");
                            var s=Number(dataTime['start'].split(':')[0])*60+Number(dataTime['start'].split(':')[1]),e=Number(dataTime['end'].split(':')[0])*60+Number(dataTime['end'].split(':')[1]),m=e-s,i=0,html='';
                            while (i<=m) {
                                html+='<li data-time="'+helper.parseHourse(s+i)+'">'+helper.parseHourse(s+i)+'</li>';
                                i=i+60;
                        ...
                        default: {
                            dataTime = getTime(dataJSON["date"]["day"], dataJSON, "Послепослезавтра");
                            var s=Number(dataTime['start'].split(':')[0])*60+Number(dataTime['start'].split(':')[1]),e=Number(dataTime['end'].split(':')[0])*60+Number(dataTime['end'].split(':')[1]),m=e-s,i=0,html='';
                            while (i<=m) {
                                html+='<li data-time="'+helper.parseHourse(s+i)+'">'+helper.parseHourse(s+i)+'</li>';
                                i=i+60;
